...
// Compress with zlib's encoder (works on every Node version) so the test also
// proves our inflater interops with a real, independent DEFLATE implementation.
...
describe("pure-JS DEFLATE inflate", () => {
  it("round-trips highly repetitive data (back-references)", async () => {
    ...
    const out = inflateRawJS(deflateRaw(input));
  ...
  it("round-trips an empty input", async () => {
    const out = inflateRawJS(deflateRaw(new Uint8Array(0)));
